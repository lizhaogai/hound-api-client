...
class Company {
   ...
   *
   * @method
   * @path [GET] /Companies/{id}/socialGroup/count
   * @name Company#socialGroupCount
   * @param {string} id - Company id
   * @param {object|function|*} [opts] - Optional params
   * @param {string} [opts.type] - 只查询省公司(true)或者只查询市公司(false),查全部就不传
   * @param {string} [opts.filter] - 过滤
   * @param {function} [cb] - Callback function
   */
  socialGroupCount(id, opts, cb) {
    // verify the required parameter 'id' is set
    if (id === undefined || id === null) {
      throw new Error("Missing the required parameter 'id' when calling socialGroupCount");
    }
    if (typeof opts === 'function') {
      cb = opts;
      opts = null;
    ...
    const path = utils.buildPath('/Companies/{id}/socialGroup/count', {
      id: id,
    });
